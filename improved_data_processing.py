...
class GameEventProcessor:
    ...
    SPECIAL_TOKENS = {
        ...
        "SUB_EVENT": "<SUB>",
        "SHOT_EVENT": "<SHOT>",
        "REBOUND_EVENT": "<REBOUND>",
        "FREETHROW_EVENT": "<FREETHROW>",
        "PLAYER_START": "<PLAYER>",
        "TEAM_START": "<TEAM>",
        "SCORE_START": "<SCORE>",
        "TIME_START": "<TIME>",
    ...
    def parse_game_state(self, state_line: str) -> Dict[str, Any]:
        ...
        # Get player lists (until next keyword or end)
        home_players = []
        away_players = []
    ...
    def create_custom_tokenizer(self, save_path: str = "improved_tokenizer.json"):
        """Create a custom tokenizer with special tokens and learned vocabulary"""
        ...
        for team in sorted(self.team_names):
            vocab[team] = len(vocab)
        
        # Add player names
        for player in sorted(self.player_names):
            vocab[player] = len(vocab)
        
        # Add action types
        for action in sorted(self.action_types):
            vocab[action] = len(vocab)
        
        # Add common game tokens
        common_tokens = ["Made", "Missed", "Home", "Away", "OnCourtHome", "OnCourtAway",
                        "SubIn", "SubOut", "Shot", "3PT", "2PT", "Rebound",
                        "OffensiveRebound", "DefensiveRebound", "Block", "Assist",
                        "NoAssist", "NoBlock", "Q1", "Q2", "Q3", "Q4", "OT"]
...
def create_improved_dataset(game_data: List[Dict[str, Any]]) -> Dataset:
...
def main():
    ...
    tokenizer = processor.create_custom_tokenizer()
    
    # Create dataset
    dataset = create_improved_dataset(game_data)
    ...
    print(f"Tokenizer saved to improved_tokenizer.json")
    print(f"\nDataset statistics:")
